...
def download_list_of_videos(videos: (str, str), output_folder_path: Path):
    for filename, url in videos:
        ...
        if not (Path(output_file_path.as_posix() + ".lock").exists()  # check if lock exists
            ...
            Process(target=download_video, args=(filename, url, output_file_path)).start()
...
#     temporary_path = Path(output_file_path, filename)
...
def download_video(filename: str, playlist_url: str, output_file_path: Path):
    download_command = "ffmpeg -y -hide_banner -hwaccel auto " + \
                       f"-i \"{playlist_url}\" -c copy -f mp4 \"{output_file_path}\" > /dev/null"
    print("Starting download of " + filename)
    download_start_time = time.time()
    os.system(download_command)
    print(f"Download of {filename} completed after {str(time.time() - download_start_time)}s")
